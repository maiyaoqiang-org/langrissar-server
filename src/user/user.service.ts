...
export class UserService {
  private captchaStore: Map<
    string,
    { text: string; expireAt: Date; phone?: string }
  > = new Map();
  ...
  async register(createUserDto: CreateUserDto) {
    ...
    if (
      !captchaData ||
      captchaData.text !== createUserDto.captcha.toUpperCase() ||
  ...
  async login(loginDto: LoginDto) {
    ...
    if (
      !captchaData ||
      captchaData.text !== loginDto.captcha.toUpperCase() ||
      captchaData.expireAt < new Date()
    ) {
  ...
  async queryInvitationCodes(userId: number, queryDto: QueryInvitationCodeDto) {
    ...
    const query = this.invitationCodeRepository
      .createQueryBuilder("invitationCode")
    ...
    if (queryDto.isUsed) {
      query.andWhere("invitationCode.isUsed = :isUsed", {
        isUsed: queryDto.isUsed,
      });
    }

    if (queryDto.createdById) {
      query.andWhere("invitationCode.createdById = :createdById", {
